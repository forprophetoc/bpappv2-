      ...
      <div className="space-y-3">
        ...
        {filteredJobs.map((job) => {
          ...
          return (
            <div
              key={job.id}
              className="bg-white rounded-xl border border-gray-200 shadow-sm px-4 sm:px-6 py-4 flex items-center justify-between hover:shadow-md transition-shadow cursor-pointer gap-2"
              onClick={() => window.location.href = `/estimate/${job.slug}`}
            >
              <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                <div className="w-9 sm:w-10 h-9 sm:h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 font-semibold text-sm shrink-0">
                  {initials}
                </div>
