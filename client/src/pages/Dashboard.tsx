  ...
  return (
    <div className="p-4 sm:p-8">
      ...
      <div className="flex items-start justify-between mb-6 sm:mb-8">
        ...
        <Link
          href="/new-estimate"
          className="inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-semibold px-3 sm:px-5 py-2.5 rounded-lg text-sm transition-colors shadow-sm"
        >
        ...
        <div className="divide-y divide-gray-100">
          ...
          {recentJobs.map((job) => {
            ...
            return (
              <div
                key={job.id}
                className="flex items-center justify-between px-4 sm:px-6 py-3.5 hover:bg-gray-50 transition-colors cursor-pointer gap-2"
                onClick={() => window.location.href = `/estimate/${job.slug}`}
              >
                ...
                <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                  ...
                  <select
                    value={status}
                    onChange={(e) => { e.stopPropagation(); updateStatus.mutate({ id: job.id, status: e.target.value as any }); }}
                    className={`text-xs font-medium px-2 py-1 rounded-full border-none outline-none cursor-pointer hidden sm:block ${statusClass}`}
                  >
                    {JOB_STATUSES.map((s) => (
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-4 sm:px-5 py-3 sm:py-4 flex items-start justify-between">
      <div>
